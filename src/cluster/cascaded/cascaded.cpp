...
vector<SuperBlockId> cluster(shared_ptr<SequenceFile>& db, const shared_ptr<BitVector>& filter, const SuperBlockId* member_counts, bool last_round) {
	...
	if (config.mutual_cover.present()) {
		config.query_cover = config.subject_cover = config.mutual_cover + (last_round ? 0.0 : config.coverage_increment);
	}
	else {
	...
	const int ccd = config.lin_stage1 ? 0 : config.connected_component_depth;
