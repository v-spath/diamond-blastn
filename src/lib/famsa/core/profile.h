...
public:
	...
	LocalAln AlignProfProfLocal(CProfile* profile1, CProfile* profile2, vector<int>* column_mapping1, vector<int>* column_mapping2);
	std::pair<size_t, size_t> Backtrace(CProfile* profile1, CProfile* profile2, CDPMatrix& matrix, const std::vector<std::vector<bool>>& zeros, size_t max_i, size_t max_j, uint32_t no_threads = 1);
	size_t card;
};
