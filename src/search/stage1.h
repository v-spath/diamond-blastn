...
static void all_vs_all_mutual_cov(const SeedLoc* q, const SeedLoc* s, const FingerPrint* a, uint32_t na, const FingerPrint* b, uint32_t nb, FlatArray<uint32_t>& out, unsigned hamming_filter_id, WorkSet& work_set) {
	uint32_t j0 = 0, j1 = 0;
...
static void all_vs_all_self_mutual_cov(const SeedLoc* q, const FingerPrint* a, uint32_t na, FlatArray<uint32_t>& out, unsigned hamming_filter_id, WorkSet& work_set) {
#ifdef KEEP_TARGET_ID
	const double mlr = work_set.cfg.min_length_ratio;
	for (uint32_t i = 0; i < na; ++i) {
		const FingerPrint e = a[i];
...
static void FLATTEN stage1_self_mutual_cov(const SeedLoc* q, int32_t nq, const SeedLoc* s, int32_t ns, WorkSet& work_set)
{
#ifdef __APPLE__
	thread_local Container vs;
#else
...
static void FLATTEN stage1_mutual_cov_query_lin(const SeedLoc* q, int32_t nq, const SeedLoc* s, int32_t ns, WorkSet& work_set)
{
	const double mlr = work_set.cfg.min_length_ratio;
...
#else
	throw std::runtime_error("Not compiled with KEEP_TARGET_ID");
#endif
}

#ifdef KEEP_TARGET_ID

static void FLATTEN stage1_mutual_cov(const SeedLoc* q, int32_t nq, const SeedLoc* s, int32_t ns, WorkSet& work_set)
{
#ifdef __APPLE__
	thread_local Container vq, vs;
#else
...
typedef void Stage1Fn(const ::SeedLoc*, ::int32_t, const ::SeedLoc*, ::int32_t, ::Search::WorkSet&);

static Stage1Fn* stage1_dispatch(const Search::Config* cfg) {
#ifdef KEEP_TARGET_ID
